...
async def github_get_file(repo: str, path: str, ref: str = "main") -> Dict[str, str]:
    ...
    if not api_key:
        raise GitHubToolError("GITHUB_TOKEN environment variable is not set")
...
@mcp.tool()
async def github_list_issues(repo: str, state: str = "open", labels: str = "") -> List[Dict[str, Any]]:
    """
...
@mcp.tool()
async def github_create_issue(repo: str, title: str, body: str, labels: Optional[List[str]] = None) -> Dict[str, Any]:
    """
...
async def github_user_activity(username: str, days: int = 7, token: str = None) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if user_response.status_code != 200:
            return {"error": f"GitHub API error: User not found or API limit reached ({user_response.status_code})"}
        ...
        if issues_response.status_code == 200:
            for issue in issues_response.json().get("items", []):
                issues_opened.append({
                    ...
                    "repo": issue.get("repository_url").split("/repos/")[1] if "repository_url" in issue else "unknown"
                })
        ...
        if comments_response.status_code == 200:
            for item in comments_response.json().get("items", []):
                comment_info = {
                    ...
                    "repo": item.get("repository_url").split("/repos/")[1] if "repository_url" in item else "unknown"
                }
        ...
        if reviews_response.status_code == 200:
            for review in reviews_response.json().get("items", []):
                pr_reviews.append({
                    ...
                    "repo": review.get("repository_url").split("/repos/")[1] if "repository_url" in review else "unknown"
                })
