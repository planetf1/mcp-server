...
class TestGitHubTools:
    ...
    def setup(self, mock_httpx_client, GITHUB_PERSONAL_ACCESS_TOKEN_env):
    ...
    async def test_github_get_file(self):
        ...
        with patch("httpx.AsyncClient", return_value=async_cm):
            ...
            assert (
                args[0] == "https://api.github.com/repos/testuser/testrepo/contents/test/test.txt"
            )
    ...
    async def test_github_create_issue(self):
        ...
        with patch("httpx.AsyncClient", return_value=async_cm):
            result = await github_create_issue(
                "testuser/testrepo", "New Issue", "This is a new issue", ["bug", "enhancement"]
            )
    ...
    async def test_github_user_activity(self):
        ...
        issues_response.json = AsyncMock(
            return_value={
                "items": [
                    {
                        "title": "Issue Title",
        ...
        comments_response.json = AsyncMock(
            return_value={
                "items": [
                    {
        ...
        reviews_response.json = AsyncMock(
            return_value={
                "items": [
                    {
                        "title": "Reviewed PR",
