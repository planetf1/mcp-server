...
if not os.path.exists(MCP_SERVER_BINARY_PATH) or not os.path.isfile(MCP_SERVER_BINARY_PATH):
        print(f"Error: The specified MCP server binary path does not exist or is not a file.", file=sys.stderr)
        print(f"Path configured: '{MCP_SERVER_BINARY_PATH}'", file=sys.stderr)
        print(f"Please download the binary from https://github.com/github/github-mcp-server/releases", file=sys.stderr)
...
if not os.access(MCP_SERVER_BINARY_PATH, os.X_OK):
        ...
        print(f"Please make it executable (e.g., using 'chmod +x {MCP_SERVER_BINARY_PATH}')", file=sys.stderr)
        sys.exit(1)
...
async def get_llm_response(messages: List[Dict[str, str]], model: str = OLLAMA_MODEL) -> Optional[str]:
        """
...
async def chat_interaction(session: ClientSession) -> None:
        ...
        while True:
                ...
                if llm_output is None:
                        print("Assistant: Sorry, I encountered an error trying to process that. Please try again.")
                        messages.pop()
                        continue
                ...
                try:
                        ...
                        if command == "SEARCH_REPOS" and len(command_parts) >= 2:
                        ...
                        elif command == "GET_FILE" and len(command_parts) == 3: # Expecting GET_FILE owner/repo path
                                ...
                                if len(owner_repo_parts) == 2:
                                        ...
                                        print(f"Assistant: Invalid owner/repo format '{owner_repo_str}'. Use owner/repo.")
                                        error_occurred = True

                        elif command == "LIST_ISSUES" and len(command_parts) == 2: # Expecting LIST_ISSUES owner/repo
                ...
                if executed_command:
                        ...
                        try:
                                ...
                                messages.append({"role": "system", "content": f"MCP Tool '{tool_name}' Response: {json.dumps(mcp_result, default=str)}"})
                        except TypeError:
                                print(str(mcp_result))
                                messages.append({"role": "assistant", "content": llm_output})
                                messages.append({"role": "system", "content": f"MCP Tool '{tool_name}' Response: {str(mcp_result)}"})
...
async def main():
        ...
        if github_token:
                ...
                print("Error: GITHUB_PERSONAL_ACCESS_TOKEN environment variable not found.", file=sys.stderr)
        ...
        try:
                ...
                async with stdio_client(server_params) as (read, write):
                        ...
                        async with ClientSession(read, write) as session:
                                ...
                                if await get_llm_response([{"role": "user", "content": "Hello"}]) is None:
                                        print(f"Error: Cannot reach Ollama server/model {OLLAMA_MODEL}. Please ensure it's running.", file=sys.stderr)
                                        sys.exit(1)
                ...
                print(f"Please check the path '{MCP_SERVER_BINARY_PATH}', ensure it's executable, and that the required GITHUB_PERSONAL_ACCESS_TOKEN environment variable is set correctly *before running this script*.", file=sys.stderr)
                sys.exit(1)
